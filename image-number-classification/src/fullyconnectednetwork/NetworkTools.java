...
public class NetworkTools {
    ...
    public static double[] createRandomArray(Random rnd, int size, double lower_bound, double upper_bound){
        if(size < 1){
            return null;
        }
        double[] ar = new double[size];
        for(int i = 0; i < size; i++){
            ...
            //ar[i] = rnd.nextDouble() % (upper_bound-lower_bound) + lower_bound;
            //System.out.println(ar[i]);
        }
        return ar;
    }

    public static double[][] createRandomArray(Random rnd, int sizeX, int sizeY, double lower_bound, double upper_bound){
        if(sizeX < 1 || sizeY < 1){
            return null;
        }
        double[][] ar = new double[sizeX][sizeY];
        for(int i = 0; i < sizeX; i++){
            ar[i] = createRandomArray(rnd, sizeY, lower_bound, upper_bound);
        }
        return ar;
    }
